...
// --------------------------------------------------------------------------
// HdUsdBrepTessellationProcedural
// --------------------------------------------------------------------------

HdUsdBrepTessellationProcedural::HdUsdBrepTessellationProcedural(
    const SdfPath &proceduralPrimPath)
    : HdGpGenerativeProcedural(proceduralPrimPath)
{
}

HdUsdBrepTessellationProcedural::~HdUsdBrepTessellationProcedural() = default;

HdGpGenerativeProcedural::DependencyMap
HdUsdBrepTessellationProcedural::UpdateDependencies(
    const HdSceneIndexBaseRefPtr &inputScene)
{
    ...
    deps[_GetProceduralPrimPath()] = HdDataSourceLocatorSet(
        HdDataSourceLocator(_tokens->usdBrepTessellatorData));
    return deps;
}

void
HdUsdBrepTessellationProcedural::_Tessellate(
    const HdSceneIndexBaseRefPtr &inputScene)
{
    _meshes.clear();
    ...
    if (!prim.dataSource) {
        TF_WARN("HdUsdBrepTessellationProcedural: No data source for '%s'",
                primPath.GetText());
        return;
    }
    ...
    HdContainerDataSourceHandle tessDs =
        HdContainerDataSource::Cast(
            prim.dataSource->Get(_tokens->usdBrepTessellatorData));

    if (!tessDs) {
        TF_WARN("HdUsdBrepTessellationProcedural: No tessellation data for '%s'. "
    ...
    if (meshCount <= 0) {
        TF_WARN("HdUsdBrepTessellationProcedural: meshCount=0 for '%s'",
                primPath.GetText());
        return;
    }
    ...
    for (int i = 0; i < meshCount; ++i) {
        ...
        if (!meshDs) {
            TF_WARN("HdUsdBrepTessellationProcedural: Missing mesh data "
...
HdUsdBrepTessellationProcedural::Update(
    const HdSceneIndexBaseRefPtr &inputScene,
...
HdSceneIndexPrim
HdUsdBrepTessellationProcedural::GetChildPrim(
    const HdSceneIndexBaseRefPtr &inputScene,
...
HdUsdBrepTessellationProceduralPlugin::HdUsdBrepTessellationProceduralPlugin()
    = default;
...
HdUsdBrepTessellationProceduralPlugin::Construct(
    const SdfPath &proceduralPrimPath)
{
    return new HdUsdBrepTessellationProcedural(proceduralPrimPath);
}
