...
///
/// \code
///     gprim.GetMyTokenValuedAttr().Set(UsdProfilesTokens->profilesProfile);
/// \endcode
struct UsdProfilesTokensType {
    USDPROFILES_API UsdProfilesTokensType();
    /// \brief "profiles:capabilities"
    ///
    /// UsdProfilesProfileAPI
    const TfToken profilesCapabilities;
    /// \brief "profiles:profile"
    ///
    /// UsdProfilesProfileAPI
    const TfToken profilesProfile;
    /// \brief "ProfileAPI"
    ///
    /// Schema identifier and family for UsdProfilesProfileAPI
    const TfToken ProfileAPI;
